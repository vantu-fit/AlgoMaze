...
class SokobanSolver:
    ...
    def ucs_search(self):
        ...
        open_list = []
        f = self.initial_state['cost_so_far']
        heapq.heappush(open_list, (f, counter, self.initial_state))
        closed_set = set()

        while open_list:
            _, _, current_state = heapq.heappop(open_list)
            current_state_key = (current_state['ares_pos'], frozenset(
                current_state['stones'].items()))
                        ...
                        if (new_ares_x, new_ares_y) in current_state['stones']:
                            ...
                            if 0 <= new_stone_x < self.grid_width and 0 <= new_stone_y < self.grid_height:
                                if (new_stone_x, new_stone_y) not in self.walls and (new_stone_x, new_stone_y) not in current_state['stones']:
                                    ...
                                    weight = new_stones.pop(
                                        (new_ares_x, new_ares_y))
                                    new_stones[(
                                        new_stone_x, new_stone_y)] = weight
                                    action = action_char.upper()
                                    new_total_weight = current_state['total_weight'] + weight
                                    new_actions = current_state['actions'] + [
                                        action]
...
if __name__ == "__main__":
    ...
    for input_filename in os.listdir(input_dir):
        ...
        if result:
            steps, total_weight, nodes_generated, time_taken, memory_used, solution = result
            algorithm_name = "UCS"
            solver.write_output(output_filename, algorithm_name, steps,
                                total_weight, nodes_generated, time_taken, memory_used, solution)
        else:
            with open(output_filename, 'w') as f:
                f.write("No solution found.\n")
    end_time = time.time()
